...
degree_of_knowledge_examples = [
    "no knowledge required", "beginner", "intermediate", "advanced", "expert"]
    ...
    for job_offer_id in range(range_of_job_offers):
        ...
        for _ in range(amount_of_technologies):
            ...
            while technology_id in used_technologies:
                technology_id = fake.random_int(
                    min=1, max=range_of_technologies)
            used_technologies.add(technology_id)
            knowledge_level = fake.random_element(degree_of_knowledge_examples)
            queries.append(
...
def main():
    ...
    queries = generate_job_offer_technologies(
        range_of_job_offers, range_of_technologies)
